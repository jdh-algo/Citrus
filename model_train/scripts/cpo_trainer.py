...
class CPOTrainer(Trainer):
    ...
    def __init__(
        ...
        with PartialState().local_main_process_first():
            ...
            train_dataset = train_dataset.map(self.tokenize_row, num_proc=4)
    ...
    def cpo_loss(
        ...
        # -------------
        if self.state.global_step > 0 and self.state.global_step % 5 == 0:
            # process_index: 全局进程编号
            ...
            if global_rank % 8 == 0:
                print(
                    f"node id: {node_id}, global Rank: {global_rank}, world size: {world_size}, loss: {losses.mean().item()}, chosen_rewards: {chosen_rewards.mean().item()}, rejected_rewards: {rejected_rewards.mean().item()}")
        # -------------
